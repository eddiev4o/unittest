...
void unittest(Flt tolerance)
{
    ...
    for (int i=0; i<266; i++) {
        Vec ki = { varr[i*5+0], varr[i*5+1]};
        Vec ao = { ki[0], ki[1]};
        Vec el = { varr[i*5+2]};
        Vec eo = { varr[i*5+3], varr[i*5+4]};
        Flt ret = vecNormalize(ao);
        Flt errorRet = ret - el[0];
        errorRet = fabs(errorRet);
        Flt errorX = ao[0] - eo[0];
        Flt errorY = ao[1] - eo[1];
        errorX = fabs(errorX);
        errorY = fabs(errorY);
        printf("%3i %7.4lf %7.4lf %8.4lf %7.4lf %7.4lf",
               i+1, ki[0], ki[1], ret, ao[0], ao[1] ); 
            if (errorRet > tolerance) {
                printf(" <---error length\n");
            }
            else if (errorX > tolerance) {
                printf(" <---error X\n");
            }
            else if (errorY > tolerance) {
                printf(" <---error Y\n");
            }
            else {
                printf("\n");
